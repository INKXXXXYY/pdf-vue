...
- [x] Delete（删除）：删除选定元素（快捷键 Delete/Backspace，工具栏按钮"删除选中"）。
- [ ] Highlight（高光）
- [ ] Redact（遮挡）
...
- [ ] 规范提交信息与分支策略（约定式提交、feature/bugfix/release 分支）。
  ...
  - [x] 创建可回退快照：`snapshot-9bc3544`、`stable-reorder-export-v2`（页面重排导出功能完整实现）。
...
### 编辑选择删除按钮 (2024)
- [x] **功能需求**：在编辑选择模式下，工具栏显示"删除选中"按钮
- [x] **UI实现**：在工具栏添加条件显示的删除按钮，仅在编辑选择模式且有选中注释时启用
- [x] **逻辑实现**：
  - 添加 `hasSelectedAnnotations` 计算属性判断是否有选中注释
  - 提取公共删除函数 `deleteSelectedAnnotations()`，支持按钮和键盘调用
  - 保持现有键盘快捷键（Delete/Backspace）功能不变
- [x] **用户体验**：按钮状态与选中状态同步，操作直观明确
- [x] **文档更新**：README中添加完整的编辑选择与删除功能使用说明
